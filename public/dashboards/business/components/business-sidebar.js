...
class BusinessSidebar extends HTMLElement {
    ...
    render() {
        this.shadowRoot.innerHTML = `
            <style>
                ...
                .user-avatar { width: 48px; height: 48px; border-radius: 50%; background: linear-gradient(135deg, #3468c0, #1d4ed8); display: flex; align-items: center; justify-content: center; margin: 0 auto 1rem; color: white; font-size: 1.25rem; font-weight: 600; transition: all 0.3s ease; }
                ...
                .nav-link:hover { background: #f8fafc; color: #3468c0; border-left-color: #3468c0; }
                .nav-link.active { background: linear-gradient(135deg, rgba(52, 104, 192, 0.1), rgba(52, 104, 192, 0.05)); color: #3468c0; border-left-color: #3468c0; font-weight: 600; }
                ...
                .collapse-btn.nav-link { width: 100%; border: none; background: none; cursor: pointer; display: flex; align-items: center; justify-content: center; color: #64748b; font-size: 1.25rem; padding: 0.75rem 0; transition: background 0.2s, color 0.2s; }
                .collapse-btn.nav-link:hover { background: #f8fafc; color: #3468c0; }
                .sidebar.collapsed .collapse-btn.nav-link { transform: rotate(180deg); }
                .nav-link { position: relative; }
                .sidebar.collapsed .nav-link::after { content: attr(data-tooltip); position: absolute; left: 100%; top: 50%; transform: translateY(-50%); background: #1e293b; color: white; padding: 0.5rem 0.75rem; border-radius: 6px; font-size: 0.75rem; white-space: nowrap; opacity: 0; visibility: hidden; transition: all 0.2s ease; z-index: 1000; margin-left: 0.5rem; pointer-events: none; }
                .sidebar.collapsed .nav-link::before { content: ''; position: absolute; left: 100%; top: 50%; transform: translateY(-50%); border: 4px solid transparent; border-right-color: #1e293b; opacity: 0; visibility: hidden; transition: all 0.2s ease; z-index: 1000; margin-left: -0.25rem; pointer-events: none; }
                .sidebar.collapsed .nav-link:hover::after, .sidebar.collapsed .nav-link:hover::before { opacity: 1; visibility: visible; }
                @media (max-width: 768px) { .sidebar { transform: translateX(-100%); } .sidebar.show { transform: translateX(0); } }
                .collapse-btn { position: absolute; top: 1rem; right: 1rem; width: 36px; height: 36px; border-radius: 50%; background: #f3f4f6; border: none; color: #3468c0; cursor: pointer; display: flex; align-items: center; justify-content: center; font-size: 1.25rem; box-shadow: 0 1px 4px rgba(52,104,192,0.08); z-index: 10; transition: background 0.2s, color 0.2s, right 0.3s; }
                .collapse-btn:hover { background: #3468c0; color: #fff; }
                .sidebar.collapsed .collapse-btn { position: static; margin: 1rem auto 0.5rem auto; display: flex; justify-content: center; align-items: center; right: unset; left: unset; background: #f3f4f6; color: #3468c0; }
                .sidebar.collapsed .collapse-btn:hover { background: #3468c0; color: #fff; }
            </style>
    ...
    setupBadgeManager() {
        ...
        const checkBadgeManager = () => {
            if (window.BadgeManager) {
                console.log('BusinessSidebar: Setting up BadgeManager');
    ...
    // Update badge counts
    updateBadges(badges) {
        console.log('BusinessSidebar: Updating badges:', badges);
